...
class SortedMap<K, V> implements Map<K, V> {
  ...
  Map<RK, RV> cast<RK, RV>() {
    if(RK is K && RV is V) {
      ...
      return SortedMap<RK, RV>(
        ...
        ..addEntries(_sortedEntries.map((entry) => MapEntry(entry.key as RK, entry.value as RV)));
    }
  }
  ...
  @override
  Iterable<K> get keys => List.unmodifiable(_sortedEntries.map((entry) => entry.key));
  ...
  Map<K2, V2> map<K2, V2>(MapEntry<K2, V2> Function(K key, V value) convert) {
    return Map<K2, V2>.fromEntries(_sortedEntries.map((entry) => convert(entry.key, entry.value)));
  }
  ...
  @override
  Iterable<V> get values => List.unmodifiable(_sortedEntries.map((entry) => entry.value));
